...
import CompatibleTypes from '@/components/results/CompatibleTypes';
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  try {
    ...
    const appUrl = process.env.NEXT_PUBLIC_APP_URL ?? 'https://mini-mind-test.pages.dev';
...
export default async function ResultsPage({ params }: PageProps) {
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <div className="relative z-10 max-w-lg mx-auto w-full px-4 py-8 pb-16">
        {/* Nav */}
        ...
        {/* Celebration */}
        <div className="text-center mb-4 text-3xl" aria-hidden="true">🎉 ✨ 🎊</div>

        {/* Main result card with tabs */}
        <ResultCard result={result} meta={test.meta} />

        {/* Compatibility section */}
        <div className="mt-4">
          <CompatibleTypes result={result} allResults={test.results} testId={testId} />
        </div>
        ...
        <div className="mt-4 rounded-2xl border border-white/10 bg-white/5 backdrop-blur-sm p-5">
          <ShareButtons title={test.meta.titleKo} resultTitle={result.titleKo} />
        </div>

        {/* Retry / other tests */}
        <div className="mt-4 grid grid-cols-2 gap-3">
          <Link href={`/tests/${testId}/quiz/`}
            className={`flex items-center justify-center gap-2 py-3 rounded-2xl bg-gradient-to-r ${test.meta.gradient} text-white font-semibold text-sm shadow-lg hover:shadow-xl hover:scale-[1.02] transition-all duration-200`}>
            <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" /></svg>
            다시 하기
          </Link>
          <Link href="/"
            className="flex items-center justify-center gap-2 py-3 rounded-2xl bg-white/8 border border-white/10 text-white/70 font-semibold text-sm hover:bg-white/12 hover:text-white transition-all duration-200">
        ...
        {/* Comments */}
        <div className="mt-8">
          <CommentSection testId={testId} resultTypeId={resultId} resultEmoji={result.emoji} />
        </div>

        <div className="mt-12 text-center text-xs text-white/20 pb-4">
